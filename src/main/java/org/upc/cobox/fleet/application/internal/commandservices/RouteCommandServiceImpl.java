...
import org.upc.cobox.fleet.infrastructure.persistence.jpa.repositories.RouteRepository;
import org.upc.cobox.fleet.infrastructure.persistence.jpa.repositories.VehicleRepository;
...
public class RouteCommandServiceImpl implements RouteCommandService {
    ...
    public void handle(AddOrderToRouteCommand command) {
        ...
        if (!externalOrderService.existsOrderByOrderId(command.orderId())) {
            throw new OrderNotFoundException(command.orderId());
        }
        var orderId= new OrderId(command.orderId());
        route.addOrder(orderId);
        externalOrderService.markAsInTransitOrder(command.orderId());
        routeRepository.save(route);
    }
    ...
    public void handle(AssignVehicleToRouteCommand command) {
        ...
        double totalWeightKg = route.getOrderIds().stream()
                .mapToDouble(orderId -> externalOrderService.getOrderWeightByOrderId(orderId.orderId()))
                .sum();
